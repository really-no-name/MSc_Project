...
def main(name, mask_paths):
    ...
    # 保存融合结果
    output_path_majority = 'test/' + name + '_MV.png'
    save_mask_image(majority_result, output_path_majority)

    # 保存不确定性结果
    output_path_uncertainty = 'test/' + name + '_MV_UE.png'
    save_mask_image(uncertainty, output_path_uncertainty)

    # 保存不确定性图
    output_path_uncertainty_map = 'test/' + name + '_MV_UE_l.png'
