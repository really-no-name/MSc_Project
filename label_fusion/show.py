...
def display_images_with_overlay(name, path_A, path_B_1, path_B_2, path_B_3, path_B_4, path_B_5, path_B_6, path_C,
    ...
    for idx, other_imageUE in enumerate(other_imagesUE):
        ...
        axes[3, idx].set_title(f"{suffix} Uncertainty")
        axes[3, idx].set_xticks(np.arange(0, other_imageUE.size[0], step=height / 4))
        axes[3, idx].set_yticks(np.arange(0, other_imageUE.size[1], step=height / 4))
        axes[3, idx].grid(which='both', color='gray', linestyle='-', linewidth=0.5)
