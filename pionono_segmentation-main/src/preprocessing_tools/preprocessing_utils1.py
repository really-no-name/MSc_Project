...
#
#         # ------------------------------------------------
#         # Check if the number of masks is less than 6
#         # if len(masks) < 6:
#         #     print(f"{img} 无效")
#         #     continue
#         # -------------------------------------------------
...
def convert_dataset_structure(config, max_workers=default_max_workers):
    ...
    def copy_masks_and_create_list(mask_dir, out_dir):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:  # Use max_workers parameter
            results = list(executor.map(process_mask, masks_list, [mask_dir]*len(masks_list), [out_dir]*len(masks_list)))
    ...
    print('Found Train images A1: ' + str(len(train_img_list)) + ' Test images A1: ' + str(len(test_img_list1)) + ' Test images A2: ' + str(len(test_img_list2)))

    print('Copy images')
    def copy_list_of_imgs(img_list, out_dir):
        with ProcessPoolExecutor(max_workers=max_workers) as executor:  # Use max_workers parameter
            executor.map(process_img, img_list, [config['arvaniti_img_dirs']]*len(img_list), [out_dir]*len(img_list), [config['input_dir']]*len(img_list))

    copy_list_of_imgs(train_img_list, train_img_out_dir)
    copy_list_of_imgs(test_img_list1, test_img_out_dir)
    copy_list_of_imgs(test_img_list2, test_img_out_dir)

def resize_images_in_folder(config, in_dir, out_dir, resize_type='nearest', mask_fct=None, max_workers=default_max_workers):
    print('### Resize ###')
    resize_res = config['resize_resolution']
    print('Processing input: ' + in_dir + ' Output: ' + out_dir)
    os.makedirs(out_dir, exist_ok=True)
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:  # Use max_workers parameter
        executor.map(process_image_resize, img_file_list, [in_dir]*len(img_file_list), [out_dir]*len(img_file_list), [resize_res]*len(img_file_list), [interpolation]*len(img_file_list), [mask_fct]*len(img_file_list))

def resize_all_images(config, input_dir, mask_fct, max_workers=default_max_workers):
    resize_images_in_folder(config, input_dir + config['train_img_dir'], config['output_dir'] + config['train_img_dir'], 'bicubic', mask_fct=mask_fct, max_workers=max_workers)
    resize_images_in_folder(config, input_dir + config['test_img_dir'], config['output_dir'] + config['test_img_dir'], 'bicubic', mask_fct=mask_fct, max_workers=max_workers)
    for annotator_dir in config['map_annotator_dirs']:
        resize_images_in_folder(config, input_dir + config['map_dir'] + annotator_dir, config['output_dir'] + config['map_dir'] + annotator_dir, 'nearest', mask_fct=mask_fct, max_workers=max_workers)
...
def create_voting_masks(config, voting_mechanism='majority', dir_name='MV/', max_workers=default_max_workers):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:  # Use max_workers parameter
        results = list(executor.map(process_voting_image, all_imgs, [config]*len(all_imgs), [voting_mechanism]*len(all_imgs), [voting_path]*len(all_imgs)))
