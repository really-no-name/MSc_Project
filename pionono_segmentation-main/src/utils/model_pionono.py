...
class PiononoModel(nn.Module):
    ...
    def forward(self, patch, annotator_ids=None):
        """
        Get feature maps.
        """
        ...
        #---------------------------XU
        # If annotator_ids is provided, use the sample method to get predictions
        # if annotator_ids is not None:
        #     return self.sample(use_z_mean=False, annotator_ids=annotator_ids, use_softmax=True)
        # Otherwise, just return the unet features (for other use cases)
        return self.unet_features

    # def map_annotators_to_correct_id(self, annotator_ids: torch.tensor, annotator_list: list = None) -> object:
    ...
    #             print(f'Available annotators: {self.annotators}')
    #             print(f'Annotator list: {annotator_list}')
    #             print(f'Problematic annotator: {annotator_ids}')
    ...
    def map_annotators_to_correct_id(self, annotator_ids: torch.tensor, annotator_list: list = None) -> object:
        new_ids = torch.zeros_like(annotator_ids).to(device)
        for a in range(len(annotator_ids)):
            annotator_name = annotator_list[int(annotator_ids[a])]

            # 去掉前缀 "Maps/"
            if annotator_name.startswith("Maps/"):
                annotator_name = annotator_name[len("Maps/"):]

            id_corresponds = (annotator_name == np.array(self.annotators))
            if not np.any(id_corresponds):
                print(f'Available annotators: {self.annotators}')
                print(f'Annotator list: {annotator_list}')
                print(f'Problematic annotator: {annotator_name}')
                raise Exception(f'Annotator has no corresponding distribution. Annotator: {annotator_name}')
            new_ids[a] = torch.nonzero(torch.tensor(id_corresponds))[0][0]
        return new_ids
