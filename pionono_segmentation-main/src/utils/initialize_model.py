...
def init_model(annotators):
    ...
    if config['model']['method'] == 'prob_unet':
        ...
        model = ConfusionMatrixModel(num_classes=config['data']['class_no'], num_annotators=len(annotators),
                                     ...
                                     min_trace=config['model']['conf_matrix_config']['min_trace']).to(device)
    else:
        model = SupervisedSegmentationModel().to(device)
    ...
    if config['model']['load_model'] != 'None':
        ...
        if torch.cuda.is_available():
            ...
            loaded_model = torch.load(config['model']['load_model'], map_location=device)
