...
def evaluate(model, data, device, mode='test'):
    ...
    with torch.no_grad():
        ...
        for e, loader in enumerate(loader_list):
            ...
            if len(loader_list[e]) == 0:
                print(f"Warning: loader_list[{e}] is empty. Skipping this element.")
                continue

            for j, batch in enumerate(loader):
                # 无标注数据的情况下只返回图像和名称
                if len(batch) == 2:
                    test_img, test_name = batch
                    ...
                    if vis_images == 'all' or any(name in vis_images for name in test_name):
                        for k in range(len(test_name)):
                            ...
                            start2 = time.time()
                            img = test_img[k]
                            save_test_images(img, test_pred_np[k], None, test_name[k], mode)
                            save_test_image_variability(model, test_name, k, mode)
                            end2 = time.time()
                            elapsed_time2 = end2 - start2
                            print(f"Elapsed time for img {test_name[k]}: {elapsed_time2:.6f} seconds")
                            print(f"\n--------------------------图像: {test_name[k]}处理完成")

                # 有标注数据
                else:
                    test_img, test_label, test_name, ann_id = batch
                    test_img = test_img.to(device)
                    ...
                    if vis_images == 'all' or any(name in vis_images for name in test_name):
                        for k in range(len(test_name)):
                            ...
                            end2 = time.time()
                            elapsed_time2 = end2 - start2
                            print(f"\nElapsed time for img {test_name[k]}: {elapsed_time2:.6f} seconds")
                            print(f"\n--------------------------图像: {test_name[k]}处理完成")
...
if __name__ == "__main__":
    ...
    # 只加载测试数据
    _, _, test_data, annotators = get_data(load_train=False, load_val=False)
